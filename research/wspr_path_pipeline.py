...
VK7JJ_REVIEWER_ENDPOINT_CLUSTERS = (
    ("Frankfurt_1200km", 50.1109, 8.6821, 1200.0),
    ("London_1200km", 51.5072, -0.1276, 1200.0),
)
...
class FeatureRow:
    ...
    loop2_bearing_error_short: float
    loop2_bearing_error_long: float
    loop3_bearing_error_short: float
    loop3_bearing_error_long: float
...
def bearing_error_deg(a: float, b: float) -> float:
...
def build_feature_rows(raw_rows: list[dict[str, str]]) -> list[FeatureRow]:
    ...
    for key, counts in sorted(grouped.items()):
        ...
        feature_rows.append(FeatureRow(
            ...
            direction_score=(loop2 - loop3) / total,
            short_path_km=sp_km,
            long_path_km=lp_km,
            rx_short_path_bearing=sp_bearing,
            rx_long_path_bearing=lp_bearing,
            loop2_bearing_error_short=bearing_error_deg(LOOP2_BEARING_DEG, sp_bearing),
            loop2_bearing_error_long=bearing_error_deg(LOOP2_BEARING_DEG, lp_bearing),
            loop3_bearing_error_short=bearing_error_deg(LOOP3_BEARING_DEG, sp_bearing),
            loop3_bearing_error_long=bearing_error_deg(LOOP3_BEARING_DEG, lp_bearing),
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for key, counts in sorted(grouped.items()):
            ...
            writer.writerow({
                ...
                "direction_score": f"{(loop2 - loop3) / total:.6f}" if total else "",
            })
...
def write_europe_daily_dominance_csv(rows: list[EuropeDailyDominanceRow], path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in rows:
            data = row.__dict__.copy()
            data["direction_score"] = f"{row.direction_score:.6f}"
            writer.writerow(data)
...
def write_europe_validation_report(
    ...
    lines.append("")
    lines.append("`absolute_margin = Loop2_count - Loop3_count`")
    lines.append("")
    lines.append("- Positive margin means Loop 2 heard more spots.")
    lines.append("- Negative margin means Loop 3 heard more spots.")
    lines.append("- Zero margin is mixed.")
    lines.append("")
...
def model_matrix(rows: list[FeatureRow]) -> tuple[list[str], list[list[float]], list[float], list[float]]:
    ...
    bands = [30, 40]  # 20m is intercept baseline.
    names = [
        "intercept",
        "short_path_km",
        "long_path_km",
    ...
    for r in rows:
        x.append([
            ...
            r.loop2_bearing_error_short / 180.0,
            r.loop2_bearing_error_long / 180.0,
            r.loop3_bearing_error_short / 180.0,
            r.loop3_bearing_error_long / 180.0,
...
def filter_training_rows(rows: list[FeatureRow]) -> list[FeatureRow]:
    # Keep all bins, but avoid singular one-row-per-grid sparsity overwhelming the
    # first model with only-local Oceania grids. The full feature CSV still keeps
    # every row for audit and later modelling.
...
def write_report(rows: list[FeatureRow], path: Path) -> None:
    ...
    lines.append("## Target")
    lines.append("")
    lines.append("`direction_score = (Loop2_count - Loop3_count) / (Loop2_count + Loop3_count)`")
    lines.append("")
    lines.append("- `+1.0` means all observations in the bin were on Loop 2 / 330 deg.")
    lines.append("- `-1.0` means all observations in the bin were on Loop 3 / 150 deg.")
    lines.append("- `0.0` means balanced loop observations.")
    lines.append("")
...
def write_europe_summary_visuals_html(
    ...
    slot_scores = build_summary_confidence_slots(confidence_rows)
    sections = []
    for band in sorted(slot_scores):
        sections.append(f"<section><h2>{band}m</h2>")
        sections.append(svg_confidence_chart(band, slot_scores[band], date_label))
        sections.append("</section>")
    doc = f"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Europe Summary Visuals</title>
<style>
  body {{ font-family: Arial, sans-serif; margin: 24px; color: #202124; background: #fff; }}
  h1 {{ margin-bottom: 4px; }}
...
<body>
<h1>Europe Summary Visuals</h1>
<p>Generated: {generated_utc_label()}</p>
<p>{html.escape(date_label)}</p>
<p>These charts summarize the validation confidence table. Each bar shows the strongest confidence level found for that UTC slot and band. See <a href="europe_summary_report.md">summary report</a> and <a href="europe_evidence_report.md">evidence ledger report</a> for row-level details.</p>
<div class="legend">
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for row, pred in zip(train_rows, preds):
            writer.writerow({
                ...
                "direction_score": f"{row.direction_score:.6f}",
                "predicted_direction_score": f"{pred:.6f}",
                "residual": f"{row.direction_score - pred:.6f}",
            })
...
def main() -> None:
    ...
    europe_daily_rows = build_europe_daily_dominance_rows(feature_rows)
    europe_confidence_rows = build_europe_confidence_rows(europe_daily_rows)
    europe_monthly_confidence_rows = build_europe_confidence_rows(europe_daily_rows, by_month=True)
    europe_theory_rows = build_europe_theory_rows(feature_rows)
    europe_chart_rows = build_europe_chart_rows(europe_theory_rows)
    europe_dusk_theory_rows = build_europe_dusk_theory_rows(feature_rows)
    europe_dusk_chart_rows = build_europe_dusk_chart_rows(europe_dusk_theory_rows)
    europe_evidence_ledger_rows = build_europe_evidence_ledger_rows(feature_rows)
    europe_review_call = "VK4EMM"
    europe_review_date = "2026-05-05"
    europe_review_slug = f"{europe_review_call.lower()}_europe"
    europe_review_title = f"{europe_review_call} Europe Evidence Ledger Report {europe_review_date}"
    europe_review_rows = [row for row in europe_evidence_ledger_rows if row.date_utc == europe_review_date]
    date_label = "Dates: " + ", ".join(sorted({row.date_utc for row in europe_daily_rows}))
    args.out_dir.mkdir(parents=True, exist_ok=True)
    write_feature_csv(feature_rows, args.out_dir / "wspr_path_features.csv")
    write_continent_summary_csv(feature_rows, args.out_dir / "loop_summary_by_continent.csv")
    write_report(feature_rows, args.out_dir / "baseline_report.md")
    write_predictions_csv(feature_rows, args.out_dir / "baseline_predictions.csv")
    write_europe_daily_dominance_csv(europe_daily_rows, args.out_dir / "europe_daily_dominance.csv")
    write_europe_confidence_csv(europe_confidence_rows, args.out_dir / "europe_validation_confidence.csv")
    write_europe_confidence_csv(
        europe_monthly_confidence_rows,
        args.out_dir / "europe_validation_confidence_by_month.csv",
    )
    write_europe_confidence_csv(europe_confidence_rows, args.out_dir / "europe_empirical_confidence.csv")
    write_europe_confidence_csv(
        europe_monthly_confidence_rows,
        args.out_dir / "europe_empirical_confidence_by_month.csv",
    )
    write_europe_theory_csv(europe_theory_rows, args.out_dir / "europe_theory_v1_greyline.csv")
    write_europe_chart_csv(europe_chart_rows, args.out_dir / "europe_chart_by_band.csv")
    write_europe_chart_html(europe_chart_rows, args.out_dir / "europe_theory_v1_charts.html", date_label)
    write_europe_dusk_theory_csv(europe_dusk_theory_rows, args.out_dir / "europe_theory_v2_dusk.csv")
    write_europe_dusk_chart_csv(europe_dusk_chart_rows, args.out_dir / "europe_dusk_chart_by_band.csv")
    write_europe_dusk_chart_html(
        europe_dusk_chart_rows,
        args.out_dir / "europe_theory_v2_dusk_charts.html",
        date_label,
    )
    write_europe_summary_visuals_html(
        europe_confidence_rows,
        args.out_dir / "europe_summary_visuals.html",
        date_label,
    )
    write_europe_evidence_ledger_csv(europe_evidence_ledger_rows, args.out_dir / "europe_evidence_ledger.csv")
    write_europe_evidence_ledger_date_csv(
        europe_evidence_ledger_rows,
        europe_review_date,
        args.out_dir / f"europe_evidence_ledger_{europe_review_date}.csv",
    )
    write_europe_evidence_report(europe_evidence_ledger_rows, args.out_dir / "europe_evidence_report.md")
    write_europe_evidence_report_html(europe_evidence_ledger_rows, args.out_dir / "europe_evidence_report.html")
    write_europe_evidence_report(europe_review_rows, args.out_dir / f"europe_evidence_report_{europe_review_date}.md")
    write_europe_evidence_report_html(europe_review_rows, args.out_dir / f"europe_evidence_report_{europe_review_date}.html")
    write_europe_evidence_ledger_date_csv(
        europe_evidence_ledger_rows,
        europe_review_date,
        args.out_dir / f"{europe_review_slug}_evidence_ledger_{europe_review_date}.csv",
    )
    write_europe_evidence_report(
        europe_review_rows,
        args.out_dir / f"{europe_review_slug}_evidence_report_{europe_review_date}.md",
        europe_review_title,
    )
    write_europe_evidence_report_html(
        europe_review_rows,
        args.out_dir / f"{europe_review_slug}_evidence_report_{europe_review_date}.html",
        europe_review_title,
    )
    write_ledger_summary_visuals_html(
        europe_review_rows,
        args.out_dir / f"europe_evidence_summary_{europe_review_date}.html",
        f"{europe_review_call} Europe Evidence Summary {europe_review_date}",
        f"Date: {europe_review_date}",
    )
    write_ledger_summary_visuals_html(
        europe_review_rows,
        args.out_dir / f"{europe_review_slug}_evidence_summary_{europe_review_date}.html",
        f"{europe_review_call} Europe Evidence Summary {europe_review_date}",
        f"Date: {europe_review_date}",
    )
    for vk7jj_raw in sorted(args.out_dir.glob("wspr_live_VK7JJ_*.csv")):
        date_match = re.search(r"(\d{4}-\d{2}-\d{2})", vk7jj_raw.name)
        if not date_match:
            continue
        vk7jj_date = date_match.group(1)
        vk7jj_rows = build_reviewer_ledger_rows(vk7jj_raw, VK7JJ_REVIEWER_ENDPOINT_CLUSTERS)
        write_reviewer_ledger_csv(vk7jj_rows, args.out_dir / f"vk7jj_reviewer_ledger_{vk7jj_date}.csv")
        write_reviewer_ledger_html(
            vk7jj_rows,
            args.out_dir / f"vk7jj_reviewer_ledger_{vk7jj_date}.html",
            f"VK7JJ Reviewer Sample Ledger {vk7jj_date}",
            f"Source: wspr.live wspr.rx rows where rx_sign = VK7JJ for {vk7jj_date} UTC, filtered to transmitter locations within 1200 km of Frankfurt or London. Directional validation is not provided in this sample.",
        )
        write_ledger_summary_visuals_html(
            vk7jj_rows,
            args.out_dir / f"vk7jj_reviewer_summary_{vk7jj_date}.html",
            f"VK7JJ Reviewer Summary {vk7jj_date}",
            f"Date: {vk7jj_date} UTC",
        )
    write_europe_validation_report(
        europe_daily_rows,
        europe_confidence_rows,
        europe_monthly_confidence_rows,
        europe_theory_rows,
        europe_chart_rows,
        args.out_dir / "europe_summary_report.md",
    )
    write_europe_validation_report(
        europe_daily_rows,
        europe_confidence_rows,
        ...
        europe_theory_rows,
        europe_chart_rows,
        args.out_dir / "europe_validation_report.md",
    )
    write_europe_validation_report(
        europe_daily_rows,
        europe_confidence_rows,
        ...
        europe_theory_rows,
        europe_chart_rows,
        args.out_dir / "europe_empirical_report.md",
    )
    print(f"raw rows read: {len(raw_rows):,}")
    print(f"feature rows written: {len(feature_rows):,}")
    print(f"Europe daily dominance rows written: {len(europe_daily_rows):,}")
    print(f"Europe confidence rows written: {len(europe_confidence_rows):,}")
    print(f"Europe monthly confidence rows written: {len(europe_monthly_confidence_rows):,}")
    print(f"Europe theory v1 rows written: {len(europe_theory_rows):,}")
    print(f"Europe chart rows written: {len(europe_chart_rows):,}")
    print(f"Europe theory v2 dusk rows written: {len(europe_dusk_theory_rows):,}")
    print(f"Europe dusk chart rows written: {len(europe_dusk_chart_rows):,}")
    print(f"Europe evidence ledger rows written: {len(europe_evidence_ledger_rows):,}")
